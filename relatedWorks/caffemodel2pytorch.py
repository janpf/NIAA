# taken from here: https://github.com/vadimkantorov/caffemodel2pytorch
# has no way to install it otherwise
...
class SGDSolver(object):
    ...
    def init_optimizer_scheduler(self):
        self.optimizer = torch.optim.SGD(
            [
                dict(params=[param], lr=self.optimizer_params["lr"] * mult.get("lr_mult", 1), weight_decay=self.optimizer_params["weight_decay"] * mult.get("decay_mult", 1), momentum=self.optimizer_params["momentum"])
                for module in self.net.children()
                for param, mult in zip(module.parameters(), module.caffe_optimization_params + [{}, {}])
...
def to_dict(obj):
    return (
        ...
        if obj is None
            ...
            f.name: converter(v) if f.label != FD.LABEL_REPEATED else list(map(converter, v))
