...
out_file = "/workspace/analysis/not_uploaded/IA2NIMA/AVA2TID/" + config.model_path.replace("/", ".") + ".txt"
...
# datasets
TID = TID2013()
TID_test = torch.utils.data.DataLoader(TID, batch_size=450, drop_last=False, num_workers=50, pin_memory=True)
...
for i, data in enumerate(TID_test):
    logging.info(f"{i}/{len(TID_test)}")
